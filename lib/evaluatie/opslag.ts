...
export type EvaluatieToegang =
  | { ok: true; activiteit: { id: string; slug: string; title: string; dateStart: Date }; deelnemerMailId: string | null }
...
export async function bepaalToegang(opts: { token?: string | null; editie?: string | null }): Promise<EvaluatieToegang> {
  if (opts.token) {
    ...
    const dm = await prisma.deelnemerMail.findUnique({
      ...
      select: { id: true, evaluatieIngevuld: true, activity: { select: activiteitSelect } },
    });
    if (!dm) return { ok: false, reden: 'onbekend' };
    if (dm.evaluatieIngevuld) return { ok: false, reden: 'al_ingevuld' };
    if (!dm.activity.evaluatieOpen) return { ok: false, reden: 'gesloten' };
    return { ok: true, activiteit: dm.activity, deelnemerMailId: dm.id };
  }
  if (opts.editie) {
    ...
    return { ok: true, activiteit: a, deelnemerMailId: null };
  }
